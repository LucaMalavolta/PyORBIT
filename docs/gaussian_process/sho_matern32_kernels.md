...
The Simple Harmonic Oscillator (SHO) and Matern-3/2 kernels are generic GP kernels commonly used to model correlated noise regardless of the specific origin. By default, their parameters are dataset-specific, but it is possible to change this behaviour using the available keywords.

```{tip}
For their general purpose, these kernels will not inherit the hyperparameters from the `activity` common model. However, it is possible to relate the hyperparameters to the rotation period and activity-decay timescale of the star 
```
