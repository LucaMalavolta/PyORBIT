...
for pp in xrange(0,n_planets):
  labels_plt = []
  plot_truths = np.zeros(3)

  M_star1_rand = np.random.normal(M_star1,0.008,n_kept)
  outplane = np.zeros([n_kept,3])
  sample_plan = np.zeros([n_kept,n_orbpams[pp]+2])
  sample_plan[:,0] = flatblobs[:n_kept,id+0]
  sample_plan[:,1] = flatblobs[:n_kept,id+1]
  sample_plan[:,2] = flatchain[:n_kept,ip+2]


  outplane[:,0] = flatblobs[:n_kept,id+0]
  ...
  if (n_orbpams[pp]==5) :
    ...
    sample_plan[:,4] = flatblobs[:n_kept,id+1]

    outplane[:,1] = flatblobs[:n_kept,id+0]
    plot_truths[1] = orbit_pam[pp,2]
    ...
    outplane[:,2] = sample_plan[:,5]
    plot_truths[2] = pams_med[5][0]
    #plot_truths[6] = pams_med[6][0]
    ...
    for ii in xrange(0,n_kept):
      ...
      sample_plan[ii,4] = np.power( (Mu_sun*np.power(sample_plan[ii,0]*seconds_in_day/(2*np.pi),2.00)/(AU_km**3)) * M_star1_rand[ii],1.00/3.00 )

    pams_med = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), \
              zip(*np.percentile(sample_plan[:,:], [15.865, 50, 84.135],axis=0)))
    plot_truths[3] = pams_med[3][0]
    plot_truths[4] = pams_med[4][0]

  print 'Planet ',pp, labels_plt
  print pams_med

  fig = triangle.corner(outplane, labels=labels_plt, truths=plot_truths)
  fig.savefig("output/"+planet_name+"_planet"+`pp`+"_RVtriangle_MassSmall_wp_wt.pdf", bbox_inches='tight')
  plt.close()
  #
  fig = triangle.corner(outplane, labels=labels_plt, truths=plot_truths, plot_contours=True, plot_datapoints=False)
  fig.savefig("output/"+planet_name+"_planet"+`pp`+"_RVtriangle_MassSmall.pdf", bbox_inches='tight')
  plt.close()
