...
if mc.starting_point_flag:
    ...
    print 'starting point manually provided'
else:
    if os.path.isfile(dir_output + 'pyde_pops.pick'):
        ...
        print 'output from PyDE retrieved'
...
if os.path.isfile(dir_output + 'emcee_MR_pops.pick'):
    print os.path.isfile(dir_output + 'pyde_pops.pick')
    meds = pickle.load(open(dir_output + 'emcee_MR_meds.pick', 'rb'))
    pops = pickle.load(open(dir_output + 'emcee_MR_pops.pick', 'rb'))
    print 'output from emcee exploratory runs retrieved'
else:
    for ii in xrange(0, 20):
        print 'emcee exploratory run #', ii
        sampler = emcee.EnsembleSampler(mc.nwalkers, mc.ndim, mc, threads=mc.nwalkers)
        population, prob, state = sampler.run_mcmc(pos, mc.MultiRun)
        max_ind = np.argmax(prob)
        #med = np.median(population, axis=0)
        med = population[max_ind,:]
        pos = np.asarray([med + 1e-4*np.random.randn(mc.ndim) for i in range(mc.nwalkers)])
        sampler.reset()

    mc.results_resumen(med)
    pickle.dump(med, open(dir_output + 'emcee_MR_meds.pick', 'wb'))
    pickle.dump(pos, open(dir_output + 'emcee_MR_pops.pick', 'wb'))
    print 'emcee exploratory runs completed'

print 'emcee'
sampler = emcee.EnsembleSampler(mc.nwalkers, mc.ndim, mc, threads=mc.nwalkers)
sampler.run_mcmc(pos, mc.nsteps, thin=mc.thin)
