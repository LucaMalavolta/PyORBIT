...
from pyorbit.keywords_definitions import keywords_common_detrending
...
class Detrending(AbstractModel):
    ...
    def initialize_model(self, mc, **kwargs):
        ...
        for keyword in keywords_common_detrending:
    ...
    def initialize_model_dataset(self, mc, dataset, **kwargs):
        ...
        for data_name in kwargs['detrending_variables']:
            ...
            try:
                ...
                if type(self.order) is dict:
                    self.pams_order[data_name] = self.order[data_name]
                else:
...
class FullDetrending(AbstractModel):
    ...
    def initialize_model_dataset(self, mc, dataset, **kwargs):
        ...
        self.pams_order = {}
        self.detrending_type = {}

        """ For plotting purposes only """
        self.interpolated[dataset.name_ref] = {}


        for detrending_key in ['polynomial_detrending', 'exponential_detrending']:

            if detrending_key not in kwargs:
                continue

            for data_name in kwargs[detrending_key]:

                self.pams_order[data_name] = {}
                self.detrending_type[data_name] = detrending_key
                ...
                try:
                    nested_dict = kwargs[detrending_key][data_name]
                    self.pams_order[data_name] = nested_dict.get('order', self.order)
                    x_zero =  nested_dict.get('x_zero', x_zero)
                except AttributeError:
                    self.pams_order[data_name] = kwargs[detrending_key][data_name]
                except TypeError:
                    if type(self.order) is dict:
                        self.pams_order[data_name] = self.order[data_name]
                    else:
    ...
    def compute(self, parameter_values, dataset, x0_input=None):
        ...
        if x0_input is None:
            ...
            for data_name, data_order in self.pams_order.items():
                ...
                temporary_trend = polynomial.polyval(dataset.ancillary[data_name]-parameter_values['x_zero_'+data_name], coeff)

                if self.detrending_type[data_name] == 'exponential_detrending':
                    if self.natural_base:
                        trend += np.exp(temporary_trend)
                    else:
            ...
            for data_name, data_order in self.pams_order.items():
                ...
                temporary_trend = polynomial.polyval(self.interpolated[dataset.name_ref][data_name](x0_input)-parameter_values['x_zero_'+data_name], coeff)
